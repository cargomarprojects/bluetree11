...
export class PayrollDetHeaderComponent implements OnInit {
  ...
  List(outputformat: string) {
    this.searchEvents.emit({ outputformat: outputformat, searchQuery: this.searchQuery, pList: null });
  }

  callbackevent(event: any) {

    if (event.action == "GENERATE") {
      this.searchEvents.emit({ outputformat: 'GENERATE', searchQuery: this.searchQuery, extractList: event.extractlist });
    }
  }
}
